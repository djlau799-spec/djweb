          ...
          <div className="pointer-events-auto relative flex flex-col overflow-hidden rounded-3xl border border-[var(--login-border-card)] bg-[var(--login-bg-card)]/80 p-8 shadow-2xl backdrop-blur-xl">
            ...
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm text-[var(--login-text-secondary)]">
                {isFirstTime
                  ? 'Authentication is enabled for the first time. Set an admin password for the workspace.'
                  : 'Valid credentials are required to access the DSA decision engine.'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <Input
                  ...
                  allowTogglePassword
                  iconType="password"
                  label={isFirstTime ? 'Admin password' : 'LoginPassword'}
                  placeholder={isFirstTime ? 'Set a password with at least 6 characters' : 'Enter password'}
                  value={password}
                ...
                {isFirstTime && (
                  <Input
                    ...
                    placeholder="Confirm the admin password again"
              ...
              {error && (
                ...
                >
                  <SettingsAlert
                    title={isFirstTime ? 'Setup failed' : 'Verification failed'}
                ...
                <div className="relative z-10 flex items-center justify-center gap-2">
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{isFirstTime ? 'Initializing...' : 'Connecting...'}</span>
                    </>
                  ) : (
                    <span>{isFirstTime ? 'Complete setup and sign in' : 'Enter workspace'}</span>
                  )}
                </div>
