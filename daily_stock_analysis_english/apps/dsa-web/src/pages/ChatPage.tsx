...
const QUICK_QUESTIONS = [
  { label: 'Analyze Kweichow Moutai with Chan theory', skill: 'chan_theory' },
  { label: 'Analysis CATL with wave theory', skill: 'wave_theory' },
  { label: 'Analyze BYD trend', skill: 'bull_trend' },
  { label: 'Analysis SMIC with box-range strategy', skill: 'box_oscillation' },
  { label: 'Analyze Tencent hk00700', skill: 'bull_trend' },
  { label: 'Analyze East Money with sentiment-cycle strategy', skill: 'emotion_cycle' },
];
...
const getMessageSkillLabel = (msg: Message): string => getMessageSkillNames(msg).join('ã€');
...
const ChatPage: React.FC = () => {
  ...
  // Get localized text (default to English)
    ...
    async (stockCode: string) => {
      ...
      try {
        if (stockInWatchlist(stockCode)) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`Removed from watchlist ${stockCode}`);
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`Added to watchlist ${stockCode}`);
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage('Operation failed, please try again');
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || 'Unable to read context compression settings');
  ...
  const updateContextCompressionenabled = useCallback(
    async (nextenabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || 'Context compressionSettingsSavefailed');
      } finally {
  ...
  // Handle follow-up from report page: ?stock=600519&name=è´µå·žèŒ…å°&recordId=xxx
  useEffect(() => {
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : ['General'];
      ...
      await startStream(payload, {
        ...
        skillName: usedSkillNames.join('ã€'),
      });
    },
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# User message' : `# AI reply${skillLabel ? ` Â· ${skillLabel}` : ''}`;
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'thinking') return last.message || 'AI is thinking...';
    ...
    if (last.type === 'generating')
      return last.message || 'Generating final analysis...';
    return 'Processing...';
  };
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = `${toolSteps.length} tool calls Â· ${totalDuration.toFixed(1)}s`;
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">Reasoning steps</span>
          <span className="text-muted-text/50">Â·</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || `Step ${step.step}: thinking`;
          ...
          text = step.message || 'Generate analysis';
    ...
    <>
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            compact
            title="NoneChat history"
            description="After you start asking questions, conversations will be saved here."
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        title="Delete chat"
        message="This chat cannot be recovered after deletion. Delete it?"
        confirmText="Delete"
        cancelText="Cancel"
        isDanger
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content="Export chat as a Markdown file">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label="Export chat as a Markdown file"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content="Send to configured notification bot/email">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: 'Sent to notification channel' }, 3000);
                        } catch (err) {
                      ...
                      aria-label="Send to configured notification bot/email"
                    >
                      {sending ? (
                        <svg
          ...
          <p className="text-secondary-text text-sm">
            Ask AI for stock analysis, skill-based trade views, and real-time decision reports.
          </p>
          {sendToast ? (
            <InlineAlert
              variant={sendToast.type === 'success' ? 'success' : 'danger'}
              title={sendToast.type === 'success' ? 'SendSuccess' : 'Send failed'}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="Start asking AI"
                  description={'Enter "analyze 600519" or "is Moutai a buy now?" and AI will use real-time data tools to generate a decision report.'}
                ...
                >
                  ...
                  >
                    {msg.role === 'assistant' && skillLabel && (
                      <div className="mb-2">
                        <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={`Skill ${skillLabel}`}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  variant="info"
                  title="Follow-up context loading"
                  message="Loading historical analysis context. You can send a follow-up now."
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  />
                  <span className="font-medium">Context compression</span>
                  <span className="text-xs text-muted-text">Save tokens in long chats</span>
                </label>
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title="Context compression settings were not saved"
            ...
            {activeStockCode && (
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="Example: analyze 600519 / is Moutai a buy now? (Enter to send, Shift+Enter for newline)"
                  disabled={loading}
                  rows={1}
