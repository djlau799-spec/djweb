...
function renderPhaseQuality(trigger: AlertTriggerItem): React.ReactNode {
  ...
  return (
    <div className="space-y-1">
      {phase ? <Badge variant="default">{phase.replace('Market phase: ', '').replace('Market phase: ', '')}</Badge> : null}
    ...
    <Card title="Trigger history" subtitle="Evaluation records" variant="bordered" padding="md">
      ...
      {!isLoading && triggers.length === 0 ? (
        <EmptyState
          ...
          title="NoneTrigger history"
          description="The background evaluator records triggered, skipped, degraded, and failed statuses; normal non-triggered checks are not written to history."
        />
      ) : null}
      {!isLoading && triggers.length > 0 ? (
          ...
          <table className="w-full min-w-[860px] text-left text-sm">
            <thead className="border-b border-border/60 text-xs uppercase text-muted-text">
              <tr>
                ...
                <th className="px-3 py-2 font-medium">Observed value</th>
