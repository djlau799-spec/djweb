...
const settingsHelpZhCN: SettingsHelpMap = {
  'settings.base.STOCK_LIST': {
    title: 'Watchlist',
    summary: 'Configure the stock-code list used by manual analysis, scheduled tasks, and notification reports.',
    usage: 'Separate multiple stock codes with English commas. A-shares can use 6-digit codes, Hong Kong stocks can use the hk prefix, and US stocks can use tickers.',
    valueNotes: [
      'Scheduled mode rereads the currently saved STOCK_LIST before each run.',
      'If --stocks is passed on the command line, it only affects that manual run and does not lock future scheduled tasks.',
      'STOCK_GROUP_N in email groups should be a subset of STOCK_LIST. It only affects recipients, not the analysis scope.',
    ],
    impact: [
      'Affects main analysis tasks, stock coverage in market reports, notification content, and historical report records.',
    ],
    notes: [
      'Do not use Chinese commas between stock codes.',
      'After editing, save the configuration so later tasks can read it.',
    ],
  },
  'settings.ai_model.LITELLM_MODEL': {
    title: 'Primary model',
    summary: 'Specifies the default LLM model used by normal analysis flows.',
    usage: 'Use provider/model format，例如 deepseek/deepseek-v4-flash、gemini/gemini-3.1-pro-preview 或 ollama/qwen3:8b。',
    valueNotes: [
      'System configuration priority is LITELLM_CONFIG > LLM_CHANNELS > legacy provider keys。',
      '如果留空，系统会尝试根据configured API keys or channel declarations自动推断。',
      'Agent 可通过 AGENT_LITELLM_MODEL specify a model separately；留空时继承Primary model。',
    ],
    impact: [
      'Affects normal stock analysis, market reviews, report generation, and Agent calls without a separate model override.',
    ],
    notes: [
      'Without a provider prefix，LiteLLM 可能无法判断应该使用哪组 API Key。',
      'Ollama local models应配合 OLLAMA_API_BASE 或 Ollama channel usage，do not misuse OPENAI_BASE_URL。',
    ],
  },
  'settings.ai_model.LLM_CHANNELS': {
    title: 'LLM channel list',
    summary: 'Declare multiple model channels for multiple providers, multiple keys, fallback models, and visual channel management.',
    usage: 'Enter comma-separated channel names，例如 deepseek,aihubmix；then configure each channel with LLM_<NAME>_BASE_URL、LLM_<NAME>_API_KEY(S)、LLM_<NAME>_MODELS 等字段。',
    valueNotes: [
      'After enabling channel mode，同层运行时channel configuration is read first。',
      'Environment variables injected in Docker or GitHub Actions override the .env written by the web Settings page.',
      'The channel editor only updates keys included in this save and does not silently migrate all legacy configuration.',
    ],
    impact: [
      '影响Primary model、Agent Models、fallback Models和 Vision Models的可选Source。',
    ],
    notes: [
      'Do not mix minimal legacy keys with channels and expect both paths to apply simultaneously.',
      'Custom channel names usually require explicit environment-variable mapping in GitHub Actions workflows.',
    ],
  },
  'settings.ai_model.AGENT_LITELLM_MODEL': {
    title: 'Agent Primary model',
    summary: '为Ask AI、Strategy Agent 等 Agent 链路specify a model separately。',
    usage: '使用 provider/model 格式。留空时继承普通AnalysisPrimary model；裸Models名会按兼容逻辑归一到 openai/<model>。',
    valueNotes: [
      'Useful for giving Agents stronger reasoning or longer-context models.',
      '该字段只影响 Agent 链路，不会改变普通个股Analysis的Primary model。',
    ],
    impact: ['Affects model selection for Agent Q&A, strategy selection, and related tool calls.'],
    notes: ['Confirm this model is available through an enabled channel, YAML route, or legacy provider key.'],
  },
  'settings.ai_model.LITELLM_FALLBACK_MODELS': {
    title: 'Fallback models',
    summary: 'Primary modelfailed时按顺序尝试的Fallback models列表。',
    usage: 'Separate multiple models with English commas; provider/model format is still recommended.',
    valueNotes: [
      'Fallback models只在Primary modelfailed、超时或响应不可用时使用。',
      'The channel editor removes unreachable hosted-provider model references when saving.',
    ],
    impact: ['Improves availability when LLM calls fail, but can increase cross-provider cost and response differences.'],
    notes: ['不要把Primary model重复加入Fallback models列表。'],
  },
  'settings.ai_model.LITELLM_CONFIG': {
    title: 'Advanced model routing YAML',
    summary: 'Specify a native LiteLLM YAML routing file for complex routing, rate limits, and expert configuration.',
    usage: 'Enter a YAML file path accessible to the project，例如 ./litellm_config.yaml。',
    valueNotes: [
      'If it is parseable and contains model_list, it has higher priority than LLM_CHANNELS and legacy provider keys.',
      'The web channel editor does not modify the YAML file itself.',
    ],
    impact: ['Affects model selection, routing, fallback, and available-model declarations.'],
    notes: ['If YAML configuration fails, the system falls back to channel or legacy configuration paths.'],
  },
  'settings.ai_model.LLM_TEMPERATURE': {
    title: 'Temperature',
    summary: 'Controls model output randomness.',
    usage: 'Range 0.0 to 2.0; lower is more stable, higher is more varied.',
    valueNotes: [
      '0 is better for stable structured output.',
      '0.7 is the general default, balancing stability and expressiveness.',
    ],
    impact: ['Affects analysis text, report tone, and structured-output stability.'],
    notes: ['Providers may support different temperature ranges.'],
  },
  'settings.ai_model.provider_keys': {
    title: 'Model service API key',
    summary: 'Configure the access key for a model provider or aggregation gateway.',
    usage: 'Create an API key in the provider console and enter it here. For rotation or load balancing, separate multi-key fields with English commas.',
    valueNotes: [
      'Key fields are shown as password controls on the web Settings page and usually display only masks after saving.',
      'Channel mode reads first from LLM_<NAME>_API_KEY(S)，legacy key 主要用于兼容旧Configuration。',
    ],
  ...
  'settings.ai_model.anspire_llm': {
    ...
    summary: '使用 Anspire API Key 作为 OpenAI-compatible Models网关的兼容入口。',
    usage: 'ANSPIRE_LLM_ENABLED 控制YesNoEnable该兼容路径；ANSPIRE_LLM_BASE_URL 指定网关地址；ANSPIRE_LLM_MODEL 指定未显式选择Primary model时的默认Models。',
    valueNotes: [
      '该路径主要用于兼容未Configuration LLM_CHANNELS 或 LITELLM_MODEL 的简化场景。',
      '如果已Configuration LITELLM_CONFIG、LLM_CHANNELS 或明确的 LITELLM_MODEL，运行时会按既有优先级选择ModelsSource。',
    ],
    impact: ['影响 Anspire API Key 参与 LLM 调用时的默认Models和网关地址。'],
    notes: ['不要把 Base URL 改成其他 provider 的地址后继续复用 Anspire Key。'],
  },
  'settings.ai_model.legacy_provider_params': {
    title: 'Legacy Provider 参数',
    summary: '为旧版 provider 专用Configuration路径SettingsModels名、温度或 token 上限。',
    usage: '这些字段用于兼容HistoryConfiguration；新Configuration优先使用 LITELLM_MODEL、LITELLM_FALLBACK_MODELS、VISION_MODEL、LLM_TEMPERATURE 或 LLM Channels。',
    valueNotes: [
      'Enable LLM Channels 后，相关 legacy 字段通常会从General表单隐藏。',
      '字段只影响对应 provider 的 legacy 路径，不会自动迁移到ChannelConfiguration。',
    ],
    impact: ['影响未使用Channel/YAML 路由时的 legacy provider Models选择和采样参数。'],
    notes: ['如果同时维护 legacy 字段和 Channels，请以Settings page实际显示的运行时ModelsSource为准。'],
  },
  'settings.ai_model.OPENAI_BASE_URL': {
    ...
    impact: ['影响 OpenAI-compatible legacy Configuration路径下的Models请求。'],
    notes: ['Channel模式下优先维护各Channel自己的 LLM_<NAME>_BASE_URL。'],
  },
  'settings.data_source.TUSHARE_TOKEN': {
    title: 'Tushare Token',
    summary: '用于访问 Tushare Pro 数据接口。',
    usage: '在 Tushare Account中获取 token 后填入。',
    valueNotes: ['不同 Tushare 权限会影响可用接口和数据完整度。'],
    impact: ['影响部分 A 股基础数据、Stock列表和相关增强数据获取。'],
    notes: ['不要把 token 提交到仓库或公开日志。'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
    summary: '用于增强Market review中的指数、市场统计等数据。',
    usage: '在 TickFlow 获取 API Key 后填入；未Configuration时系统会继续使用其他可用数据源和降级路径。',
    valueNotes: ['该 Key Yes可选增强项，不Yes运行主Analysis流程的必填项。'],
    impact: ['影响Market review和市场统计增强数据的覆盖度。'],
    notes: ['不要在 issue、日志或截图中暴露真实 Key。'],
  },
  'settings.data_source.stock_index_remote': {
    title: 'Stock索引远程Update',
    summary: '从 GitHub main 分支获取最新Stock自动补全索引，并缓存到本地。',
    usage: '默认开启；如运行环境无法访问 GitHub raw，可Close开关。远程 URL、检查频率和超时Time均为系统内置值。',
    valueNotes: ['系统默认 48 小时检查一次Update，避免频繁访问 GitHub。', '远程检查failed不会阻断 WebUI 或Analysis流程。'],
    impact: ['影响 Web 自动补全和后端StockName解析使用的Stock简称新鲜度。'],
    notes: ['远程Downloadfailed时会继续使用已有缓存或随应用打包的内置索引。'],
  },
  'settings.data_source.ALPHASIFT_ENABLED': {
    title: 'AlphaSift Screening',
    summary: '控制YesNoEnable内置 AlphaSift Screeningpage。',
    usage: '默认Close。设为 true 后，Web 会检查随后端依赖安装的 alphasift.dsa_adapter；若缺失，请先执行 pip install -r requirements.txt 或重建后端产物。',
    valueNotes: ['AlphaSift 作为 DSA 后端依赖安装，/install 仅作为显式修复入口保留。', 'ScreeningResult仅用于研究辅助，不构成投资建议。'],
    impact: ['影响 Web Screening入口、AlphaSift Strategy读取和Screening API。'],
    notes: ['AlphaSift 初筛候选，DSA 补充行情、基本面和新闻上下文；Close时不影响原有Analysis、Report和Notification流程。'],
  },
  'settings.data_source.ALPHASIFT_INSTALL_SPEC': {
    title: 'AlphaSift 安装Source',
    summary: 'Configuration显式修复安装使用的受信任 AlphaSift pip Source。',
    usage: '默认固定到已验证的 ZhuLinsen/alphasift commit；正常部署通过 requirements 安装，只有手动调用修复安装入口时才使用该Source。',
    valueNotes: ['自定义本地路径或 wheel 不会走修复安装；请先手动安装到当前后端 Python 环境。', '该字段按敏感值处理，Settings page不会直接展示完整内容。'],
    impact: ['影响 AlphaSift 适配层Source校验和显式修复安装。'],
    notes: ['请确认Source可信；AlphaSift Yes实验性质Screening能力，Enable前应理解相关风险。'],
  },
  'settings.data_source.REALTIME_SOURCE_PRIORITY': {
    title: 'Real-time行情源优先级',
    summary: 'Configuration多个Real-time行情源的尝试顺序。',
    usage: '优先级使用英文逗号分隔；系统会按顺序尝试可用数据源。',
    valueNotes: ['前面的数据源优先使用，failed后再降级到后续数据源。'],
    impact: ['影响现价、IntradayAnalysis和依赖Real-timePrice的Report字段。'],
    notes: ['单一数据源failed应降级到后续数据源，不应拖垮主流程。'],
  },
  'settings.data_source.realtime_quotes': {
    title: 'Real-time行情Configuration',
    summary: '控制Real-time行情和Intraday技术IndicatorsYesNoEnable。',
    usage: '开关字段使用 true/false；行情源顺序由 REALTIME_SOURCE_PRIORITY 单独Configuration。',
    valueNotes: [
      'CloseReal-time行情后，Analysis会更依赖History收盘价。',
      'Real-time技术Indicators会把IntradayPrice纳入Moving average和趋势判断。',
    ],
    impact: ['影响现价、技术Indicators、IntradayAnalysis和部分Report字段。'],
    notes: ['单一数据源failed应降级到后续数据源，不应拖垮主流程。'],
  },
  'settings.data_source.search_api_keys': {
    title: '搜索服务 API Key',
    summary: 'Configuration新闻与搜索增强所需的Page三方搜索服务Key。',
    usage: '多 Key 字段使用英文逗号分隔；系统会按现有搜索优先级和可用性选择服务。',
    valueNotes: ['搜索Result用于补充新闻、公告和市场信息上下文。'],
    impact: ['影响新闻检索覆盖度、时效性和 Agent/Report中的外部信息。'],
    notes: ['搜索服务可能有额度、限流和地区可用性差异。'],
  },
  'settings.data_source.SEARXNG_BASE_URLS': {
    title: 'SearXNG 实例地址',
    summary: 'Configuration自建或可信 SearXNG 搜索实例。',
    usage: '多个实例使用英文逗号分隔；自建实例需要Enable JSON 输出格式。',
    valueNotes: ['禁用公开实例自动发现后，只会使用这里Configuration的实例。'],
    impact: ['影响无商业搜索 Key 时的新闻和网page搜索兜底能力。'],
    notes: ['公Total实例稳定性不可控，生产环境建议使用自建或可信实例。'],
  },
  'settings.data_source.ENABLE_CHIP_DISTRIBUTION': {
    title: '筹码分布Analysis',
    summary: '控制YesNoEnable筹码分布相关Analysis。',
    usage: '云部署或数据源不稳定时可设为 false。',
    valueNotes: ['Close后会减少相关数据请求和failed噪音。'],
    impact: ['影响Report中的筹码分布、成本区间等相关判断。'],
    notes: ['该能力依赖外部数据源稳定性。'],
  },
  'settings.data_source.BIAS_THRESHOLD': {
    title: '乖离率Threshold',
    summary: 'Settings股价偏离 MA5 的风险InfoThreshold。',
    usage: '填写百分比数值；当Price偏离 MA5 超过Threshold时，Report会Info避免追高或注意回归风险。',
    valueNotes: ['强趋势Stock可能按运行时Rule适当放宽Threshold。'],
    impact: ['影响技术Analyzing追高风险、Moving average偏离和操作建议的Info强度。'],
    notes: ['Threshold过低会增加风险Info噪声，过高可能弱化追高提醒。'],
  },
  'settings.data_source.pytdx': {
    title: 'Pytdx 通达信服务器',
    summary: 'Configuration通达信行情服务器地址，覆盖内置默认服务器。',
    ...
    impact: ['影响使用 Pytdx 数据源时的行情连接Target和可用性。'],
    notes: ['服务器不可达时应依赖数据源 fallback，不建议只Configuration单个不稳定地址。'],
  },
  'settings.data_source.news_window': {
    title: '新闻Time窗口',
    summary: '控制纳入Analysis上下文的新闻时效范围。',
    usage: 'NEWS_MAX_AGE_DAYS 设最大天数，NEWS_STRATEGY_PROFILE 设窗口Strategy。',
    valueNotes: ['实际窗口会受 profile 与最大天数Total同约束。'],
    impact: ['影响新闻上下文Count、时效性和Report长度。'],
    notes: ['窗口过长可能引入陈旧信息，过短可能遗漏慢发酵事件。'],
  },
  'settings.notification.FEISHU_WEBHOOK_URL': {
    title: '飞书群机器人 Webhook',
    summary: 'Configuration飞书自定义群机器人，用于把AnalysisReport推送到指定飞书群。',
    ...
    valueNotes: [
      ...
      'FEISHU_APP_ID / FEISHU_APP_SECRET 用于飞书应用、云文档或 Stream Bot，不会直接Enable群 Webhook 推送。',
    ],
    impact: [
      '影响飞书NotificationChannel；failed时不应拖垮主Analysis流程，只影响该Channel送达。',
    ],
    notes: [
      '不要把 FEISHU_APP_SECRET 当作 FEISHU_WEBHOOK_SECRET 使用。',
      '如果飞书侧Configuration IP 白名单，需要确认当前运行环境出口 IP 已加入白名单。',
    ],
  },
  'settings.notification.FEISHU_STREAM_ENABLED': {
    title: '飞书 Stream 模式',
    summary: 'Enable飞书应用机器人 / Stream Bot 长连接模式，不Yes飞书群 Webhook 推送开关。',
    usage: '只有在已创建飞书应用、completed应用发布、权限和事件订阅Configuration后才开启；同时需要 FEISHU_APP_ID 和 FEISHU_APP_SECRET。',
    valueNotes: [
      'true 表示允许运行时使用应用机器人 Stream 模式。',
      'false 表示不Enable Stream 模式；群消息推送仍应使用 FEISHU_WEBHOOK_URL。',
      '只填写 App ID/Secret 或只开启 Stream，不等于Enable群 Webhook 推送。',
    ],
    impact: [
    ...
    notes: [
      'Save后通常需要重启相关 bot/服务进程，已运行的长连接不会自动重建。',
      'failed只应影响飞书应用机器人链路，不应拖垮主Analysis流程。',
    ],
  },
  'settings.notification.FEISHU_CHAT_ID': {
    title: '飞书 App Bot 推送Target',
    summary: 'Configuration飞书应用机器人主动推送的Target chat_id（群聊模式）或 open_id（私聊模式）。',
    ...
    valueNotes: [
      '仅凭 FEISHU_APP_ID / FEISHU_APP_SECRET 不会自动Enable群 Webhook 推送。',
      'App Bot 模式与 Webhook 模式互斥：Webhook URL 优先，未Configuration Webhook 时才走 App Bot。',
    ],
    impact: [
      '影响飞书 App Bot NotificationChannel的送达Target。',
      'failed时不应拖垮主Analysis流程，只影响该Channel送达。',
    ],
    notes: [
  ...
  'settings.notification.FEISHU_RECEIVE_ID_TYPE': {
    title: '飞书接收方 ID Type',
    summary: '指定 FEISHU_CHAT_ID 的Type：chat_id 表示群聊，open_id 表示私聊。',
    usage: '群聊选择 chat_id；私聊（给指定用户发 P2P 消息）选择 open_id。',
    valueNotes: [
      '仅当 FEISHU_CHAT_ID 已填写时生效。',
      '填错Type会导致消息Send failed；如果收到 invalid receive_id Error，需要确认该值与前端的实际 ID Type一致。',
    ],
    impact: ['影响飞书 App Bot 消息的路由方式。'],
    notes: ['大多数场景使用 chat_id 即可；如果值不Yes chat_id 或 open_id，运行时会自动回退到 chat_id。'],
  },
  'settings.notification.FEISHU_DOMAIN': {
    ...
    valueNotes: [
      ...
      '选错会导致 API 调用failed（SDK 连错服务器）。',
    ],
    impact: ['影响飞书 App Bot 主动推送的 API 连通性。'],
    notes: ['如果值不Yes feishu 或 lark，运行时会自动回退到 feishu。'],
  },
  'settings.notification.DINGTALK_STREAM_ENABLED': {
    title: '钉钉 Stream 模式',
    summary: 'Enable钉钉应用机器人长连接模式，不Yes普通钉钉群机器人 Webhook 开关。',
    usage: '需要先在钉钉开放平台Configuration应用机器人，并填写 DINGTALK_APP_KEY 和 DINGTALK_APP_SECRET。',
    valueNotes: [
      'true 表示允许运行时使用钉钉应用机器人 Stream/长连接模式。',
      'false 表示不Enable该长连接模式；自定义 Webhook 中的钉钉群机器人地址仍走 CUSTOM_WEBHOOK_URLS。',
    ],
    impact: [
      '影响钉钉应用机器人交互或长连接链路。',
      '不会改变自定义 Webhook Notification的Send路径。',
    ],
    notes: [
      'Save后通常需要重启相关 bot/服务进程，已运行的长连接不会自动重建。',
      '不要把 Stream 模式和群机器人 Webhook 混为一条Configuration路径。',
    ],
  },
  'settings.notification.webhooks': {
    title: '企业微信 Webhook',
    summary: 'Configuration企业微信群机器人 Webhook，用于把AnalysisReport推送到指定群。',
    usage: '在企业微信群中创建机器人后，复制 qyapi.weixin.qq.com/cgi-bin/webhook/send 开头的 Webhook URL。',
    valueNotes: [
      'Webhook URL 通常包含敏感 token，应按Key处理。',
      '不同平台对消息长度、格式和频率限制不同。',
    ],
    impact: ['影响对应 Webhook NotificationChannel的Report送达。'],
    notes: ['单个NotificationChannelfailed不应阻断主Analysis流程。'],
  },
  'settings.notification.CUSTOM_WEBHOOK_URLS': {
    title: '自定义 Webhook',
    summary: '向任意支持 POST JSON 的服务推送Report。',
    usage: '多个 URL 使用英文逗号分隔；如需自定义 body，可Configuration CUSTOM_WEBHOOK_BODY_TEMPLATE。',
    valueNotes: [
      '模板必须渲染为 JSON object。',
      '推荐使用 $content_json、$title_json 避免换行和引号破坏 JSON。',
    ],
    impact: ['影响 AstrBot、NapCat、自建服务等自定义推送。'],
    notes: ['先用一个 Webhook 验证Success，再扩展到多个Target。'],
  },
  'settings.notification.WEBHOOK_VERIFY_SSL': {
    title: 'Webhook SSL 校验',
    summary: '控制Send HTTPS Webhook 时YesNo校验证书。',
    usage: '默认保持 true；只有可信内网自签证书场景才考虑 false。',
    valueNotes: ['Close校验会降低中间人攻击防护。'],
    impact: ['影响所有自定义 Webhook HTTPS 请求的 TLS 校验行为。'],
    notes: ['公网环境不要Close SSL 校验。'],
  },
  'settings.notification.telegram': {
    title: 'Telegram 推送',
    summary: '通过 Telegram Bot 向个人、群组或 Topic 推送Report。',
    usage: '使用 @BotFather 创建 Bot，填写 Bot Token 和Target Chat ID；群组 Topic 可额外填写 Thread ID。',
    valueNotes: ['Bot 需要被加入Target群组并具备发言权限。'],
    impact: ['影响 Telegram NotificationChannel。'],
    notes: ['群组 Chat ID 通常Yes负数或 -100 开头。'],
  },
  'settings.notification.email': {
    title: '邮件Notification',
    summary: '通过 SMTP 邮箱SendAnalysisReport。',
    usage: '填写发件邮箱、SMTP 授权码和收件人列表；多个收件人使用英文逗号分隔。',
    valueNotes: [
      'EMAIL_PASSWORD 通常Yes邮箱授权码，不Yes网pageLoginPassword。',
      '可用 STOCK_GROUP_N / EMAIL_GROUP_N Configuration分组收件人。',
    ],
    impact: ['影响邮件ReportSend、分组收件和Market review邮件送达。'],
    notes: ['不同邮箱服务商需要先开启 SMTP 服务。'],
  },
  'settings.notification.chat_bots': {
    title: '聊天平台 Bot',
    summary: 'Configuration Discord、Slack、Pushover、ServerChan 等聊天或推送平台。',
    usage: '按平台选择 Webhook 或 Bot Token 模式；Bot 模式通常还需要频道 ID。',
    valueNotes: ['同一平台同时Configuration Bot 与 Webhook 时，Code可能按既定优先级选择其中一种。'],
    impact: ['影响对应聊天平台NotificationChannel。'],
    notes: ['Bot Token、Webhook URL、SendKey 都应按Key处理。'],
  },
  'settings.notification.report_output': {
    title: 'Report输出Settings',
    summary: '控制NotificationReport的详细程度、语言和模板输出。',
    usage: 'REPORT_TYPE 可选 simple/full/brief，REPORT_LANGUAGE 可选 zh/en。',
    valueNotes: ['Report语言影响默认模板和Notification文案，不等同于前端界面语言。'],
    impact: ['影响Notification正文长度、语言和阅读体验。'],
    notes: ['full Report可能更长，部分平台可能触发分段Send。'],
  },
  'settings.system.WEBUI_HOST': {
    ...
    valueNotes: [
      ...
      '在Settings pageSave后，只会写入 .env 并重载运行时Configuration对象，不会让当前 WebUI/API 进程重新绑定监听地址。',
      'Docker Compose 中通常会在容器内使用 0.0.0.0，宿主机访问还取决于端口映射。',
    ],
    impact: [
      '影响重启后浏览器能No从本机、局域网或公网访问 WebUI。',
    ],
    notes: [
      '修改 WEBUI_HOST 后需要重启当前进程、Docker 容器或服务管理器才会生效。',
      '直连公网时建议同时Enable ADMIN_AUTH_ENABLED。',
      '如果部署在反向代理后面，Login限流与真实 IP 识别还需要Evaluated TRUST_X_FORWARDED_FOR。',
    ],
  },
  'settings.system.WEBUI_PORT': {
    ...
    valueNotes: [
      ...
      'Settings pageSave只会写入 .env，不会让当前 WebUI/API 进程重新绑定端口。',
    ],
  ...
  'settings.system.LOG_DIR': {
    ...
    notes: [
      ...
      'Docker、桌面端和本地源码运行的可写路径不同，Save前需确认权限。',
    ],
  },
  'settings.system.WEBUI_ENABLED': {
    title: '默认启动 WebUI',
    summary: '控制启动期YesNo默认进入 WebUI/API 服务模式。',
    usage: '这Yes兼容旧启动入口的启动期Configuration；Save后不会让当前page面立即启动或Close WebUI。',
    valueNotes: [
      'true 表示后续按默认入口启动时倾向进入 WebUI/API 服务模式。',
      'false 表示保持非 WebUI 默认启动行为；显式 CLI 参数仍可能覆盖该Configuration。',
    ],
    impact: [
    ...
    notes: [
      'Save后需要重启相关进程才会生效。',
      '不要把该开关理解为当前 Web Settings page的即时启停按钮。',
    ],
  },
  'settings.system.WEBUI_AUTO_BUILD': {
    title: '启动前自动构建前端',
    summary: '控制后端启动 WebUI 前YesNo自动检查并构建前端静态产物。',
    usage: '源码部署通常保持 true；已预构建镜像、离线环境或受限环境可设为 false。',
    valueNotes: [
      'true 时启动流程会尝试准备 apps/dsa-web 静态产物。',
      'false 时只检查已有构建产物；如果产物缺失，WebUI 可能不可用或只看到后端Warning。',
    ],
    impact: [
      '影响 WebUI 下一次启动时前端静态资源YesNo自动准备。',
    ],
    notes: [
      'Save后不会立即触发构建，需要重启相关后端进程。',
      '在 Docker 或发布包中Close前，请确认构建产物已经随镜像或安装包提供。',
    ],
  },
  'settings.system.ADMIN_AUTH_ENABLED': {
    title: 'Web Login保护',
    summary: 'Enable WebUI Admin password保护。',
    usage: '请通过 WebUI 的AuthenticationSettings入口Enable或Close；忘记Password可运行 python -m src.auth reset_password。',
    valueNotes: ['直连公网、局域网Total享或反向代理部署时建议Enable。', '该字段在GeneralConfigurationpage仅作说明展示，避免绕过AuthenticationSettings流程。'],
    impact: ['影响 WebUI Login、Settings page访问和管理操作保护。'],
    notes: ['Enable前请确认部署环境可以持久化Authentication数据；手动改 .env 后需要重启进程或使用AuthenticationSettings流程RefreshStatus。'],
  },
  'settings.system.TRUST_X_FORWARDED_FOR': {
    ...
    impact: ['影响Login限流、审计和真实 IP 识别。'],
    notes: ['不可信代理链上开启会允许客户端伪造Source IP。'],
  },
  'settings.system.schedule': {
    title: '定时任务',
    summary: '控制YesNoEnable每日定时Analysis以及启动时YesNo立即执行一次。',
    ...
    valueNotes: [
      ...
      'SCHEDULE_ENABLED 和 SCHEDULE_RUN_IMMEDIATELY 属于启动期行为，Save后不会启动、停止或重建当前 scheduler。',
      '定时任务触发时会读取当前Save的 STOCK_LIST。',
    ],
    impact: ['影响 schedule 模式下自动Analysis频率、启动行为和Notification推送Time。'],
    notes: [
      '注意运行环境时区，容器和服务器时区可能与本地不同。',
      '若当前进程未以 schedule 模式启动，Save这些字段不会自动创建调度器。',
    ],
  },
  'settings.system.RUN_IMMEDIATELY': {
    title: '启动后立即运行',
    summary: '控制非定时模式启动时YesNo立即执行一次Analysis。',
    usage: '需要只启动服务、不立即Analysis时设为 false。',
    valueNotes: ['与 SCHEDULE_RUN_IMMEDIATELY 分别控制非定时和定时模式。'],
    impact: ['影响服务启动后的首次Analysis行为。'],
    notes: [
      '这Yes非 schedule 模式的启动期单次运行Configuration，Save后不会让当前 WebUI/API 进程立即触发Analysis。',
      'CLI 参数和运行模式也会影响最终行为；修改后需重启非 schedule 进程后生效。',
    ],
  },
  'settings.system.TRADING_DAY_CHECK_ENABLED': {
    title: '交易日检查',
    summary: '控制非交易日YesNo跳过Analysis。',
    usage: '默认 true；需要强制运行可设为 false 或使用 --force-run。',
    valueNotes: ['会结合市场日历判断 A 股、港股、美股等市场YesNo开市。'],
    impact: ['影响定时任务和手动运行YesNo在休市日执行。'],
    notes: ['Close后休市日可能生成缺少Real-time行情的Report。'],
  },
  'settings.system.HTTP_PROXY': {
    title: '网络代理',
    summary: '为外部 API、Models服务或搜索请求Configuration代理地址。',
    usage: '填写 http://host:port 形式；HTTPS_PROXY 可用于 HTTPS 请求代理。',
    valueNotes: ['代理YesNo生效取决于底层库和运行环境变量读取方式。'],
    impact: ['影响数据源、LLM、搜索和Notification等外部网络请求。'],
    notes: ['不要把代理地址写成只在本机可见但容器不可达的 127.0.0.1。'],
  },
  'settings.llm_channel.channel_name': {
    title: 'ChannelName',
    summary: 'ChannelName用于生成 LLM_<NAME>_* 环境变量。',
    usage: '只能使用小写字母、数字和下划线；Save后会写入 LLM_CHANNELS。',
    valueNotes: ['例如 deepseek 会对应 LLM_DEEPSEEK_BASE_URL、LLM_DEEPSEEK_API_KEY(S)、LLM_DEEPSEEK_MODELS。'],
    impact: ['影响Channel环境变量命名、运行时选择和 GitHub Actions 显式映射。'],
    notes: ['修改Name不会自动迁移旧Channel的所有外部环境变量。'],
  },
  'settings.llm_channel.protocol': {
    title: 'Channel协议',
    summary: '声明该channel usage哪类兼容协议。',
    usage: 'OpenAI Compatible 适合大多数中转和兼容服务；官方 Gemini/Anthropic/DeepSeek 可选择对应协议。',
    valueNotes: ['协议会影响Models名前缀归一、连接Test和Models发现方式。'],
    impact: ['影响请求适配器、Models列表解析和运行时Models引用。'],
    notes: ['协议与 Base URL、API Key 所属服务必须匹配。'],
  },
  'settings.llm_channel.base_url': {
    title: 'Base URL',
    summary: '该Channel的接口根地址。',
    usage: 'OpenAI-compatible 服务通常填写以 /v1 结尾的地址；部分官方 SDK Channel可留空。',
    valueNotes: ['服务商预设只提供参考值，实际可用性取决于账号、地区和 provider 当前接口。'],
    impact: ['影响连接Test、Models发现和所有该Channel的 LLM 请求。'],
    notes: ['不要把不同 provider 的 API Key 和 Base URL 混用。'],
  },
  'settings.llm_channel.api_key': {
    title: 'API Key',
    summary: '该Channel调用Models服务所需的访问Key。',
    usage: '单个 Key 直接填写；多个 Key 使用英文逗号分隔。',
    valueNotes: ['本地 Ollama 等无需鉴权的服务可留空。'],
    impact: ['影响连接Test、Models发现、运行时调用和 Key 轮换。'],
    notes: ['不要在截图、日志或 issue 中暴露真实 Key。'],
  },
  'settings.llm_channel.models': {
    title: 'ChannelModels列表',
    summary: '声明该Channel可供运行时选择的Models。',
    usage: '可点击“获取Models”从支持 /models 的Channel拉取，也可手动填写逗号分隔列表。',
    valueNotes: ['Save时运行时Primary model、Agent Models、Vision Models和 fallback 会引用这里的Models。'],
    impact: ['影响可选择Models、Save前失效Models清理和运行时Models路由。'],
    notes: ['ModelsYesNo真正可用仍取决于服务商权限和运行时连接Test。'],
  },
  'settings.llm_channel.capability_checks': {
    title: '运行时能力检测',
    summary: '手动验证当前ChannelModelsYesNo支持 JSON、tools、stream 或 vision。',
    usage: '选择能力后点击检测；检测会发起真实 LLM 请求。',
    valueNotes: ['多选检测可能需要 20-40 秒，并可能消耗 provider 额度。'],
    impact: ['仅影响当前page面DiagnosticsResult，不会改变SaveConfiguration。'],
    notes: ['能力标签Yes参考，最终以检测Result和实际调用为准。'],
  },
  'settings.llm_channel.temperature': {
    title: 'Temperature',
    summary: '运行时统一采样温度。',
    usage: '滑块范围 0 到 2；低值更稳定，高值更随机。',
    valueNotes: ['Save后写入 LLM_TEMPERATURE。'],
    impact: ['影响普通Analysis、Agent 和Report生成中的Models输出稳定性。'],
    notes: ['结构化输出不稳定时优先降低 temperature。'],
  },
  'settings.llm_channel.primary_model': {
    title: 'Primary model',
    summary: '普通Analysis流程默认使用的运行时Models。',
    usage: '从enabledChannel的Models列表中选择；自动模式使用Page一个可用Models。',
    valueNotes: ['Save后写入 LITELLM_MODEL。'],
    impact: ['影响个股Analysis、Market review和默认Report生成。'],
    notes: ['如果Models不在enabledChannel列表中，Save时可能被清理或要求重新选择。'],
  },
  'settings.llm_channel.agent_primary_model': {
    title: 'Agent Primary model',
    summary: 'Agent 链路专用Primary model。',
    usage: '可选择独立Models；自动模式继承普通AnalysisPrimary model。',
    valueNotes: ['Save后写入 AGENT_LITELLM_MODEL。'],
    impact: ['影响Ask AI、Strategy Agent 和相关工具调用。'],
    notes: ['Agent Models可比普通AnalysisModels更强，但成本和延迟也可能更高。'],
  },
  'settings.llm_channel.fallback_models': {
    title: '备选Models',
    summary: 'Primary modelfailed时使用的Fallback models集合。',
    usage: '勾选一个或多个Models；Primary model不会重复加入备选Models。',
    valueNotes: ['Save后写入 LITELLM_FALLBACK_MODELS。'],
    impact: ['影响 LLM failed时的降级Success率和跨 provider 行为差异。'],
    notes: ['Fallback models应先经过连接Test或实际调用验证。'],
  },
  'settings.llm_channel.vision_model': {
    title: 'Vision Models',
    summary: '用于截图识别、图像输入或视觉相关能力的Models。',
    usage: '选择支持图像输入的Models；自动模式跟随默认 Vision 逻辑。',
    valueNotes: ['Save后写入 VISION_MODEL。'],
    impact: ['影响图片/截图相关提取和视觉Analysis能力。'],
    notes: ['DeepSeek 等文本Models不一定支持 vision，请用能力检测确认。'],
  },
  // ------------------------------------------------------------------
  // Agent configuration
  // ------------------------------------------------------------------
  'settings.agent.AGENT_MODE': {
    title: 'Agent 模式',
    summary: 'Enable ReAct Agent 进行StockAnalysis，替代普通Analysis流程。',
    usage: '开启后，系统使用多步推理 Agent 替代单轮 LLM Analysis，可调用工具、检索新闻和执行复杂推理链路。',
    valueNotes: [
      'Close时使用普通单轮 LLM Analysis模式。',
      'Agent 模式可配合 AGENT_ARCH 选择 single（经典 ReAct）或 multi（编排器 pipeline）。',
    ],
    impact: ['影响个股Analysis流程、Report生成质量和 LLM 调用次数。'],
    notes: ['Agent 模式会消耗更多 token 和Time，适合需要深度推理的场景。'],
  },
  'settings.agent.AGENT_MAX_STEPS': {
    ...
    valueNotes: [
      '步数越高，推理越深入，但Latency和 token 消耗也越大。',
      '部分复杂场景（如多Strategy编排）可能需要更高步数。',
    ],
    impact: ['影响 Agent 推理深度、Latency和 token 消耗。'],
    notes: ['设为 0 或极低值可能导致推理不完整。'],
  },
  'settings.agent.AGENT_SKILLS': {
    title: 'Agent Strategy列表',
    summary: '指定 Agent 使用的Strategy技能列表。',
    usage: '使用英文逗号分隔Strategy名；留空使用Default strategy（bull_trend）；设为 all EnableAllStrategy。',
    valueNotes: [
      'Settings具体Strategy（非 all）时，定时任务会自动使用 Agent pipeline。',
      '可用Strategy取决于 strategies/ 目录中的定义文件。',
    ],
    impact: ['影响 Agent Analysis时使用的Strategy选择和最终信号综合。'],
    notes: ['自定义Strategy需放在 AGENT_SKILL_DIR 指定的目录中。'],
  },
  'settings.agent.AGENT_SKILL_DIR': {
    title: 'Strategy目录',
    summary: '存放 Agent Strategy定义文件的目录。',
    usage: '填写相对于项目根目录的路径；目录内可放置 YAML 或 SKILL.md 格式的Strategy定义。',
    valueNotes: ['默认 strategies 目录包含内置Strategy。'],
    impact: ['影响 Agent 可发现的Strategy列表。'],
    notes: ['修改目录后需要确保新目录中包含有效的Strategy定义文件。'],
  },
  'settings.agent.AGENT_NL_ROUTING': {
    title: '自然语言路由',
    summary: '允许 bot dispatcher 通过自然语言识别将Stock查询路由到 Agent。',
    usage: '开启后，私聊中高置信度的Stock相关消息（或群聊 @机器人）会自动路由到 Agent，无需显式命令。',
    valueNotes: ['仅影响 bot 接入场景（飞书、Telegram 等），不影响 Web API。'],
    impact: ['影响 bot 交互体验和 Agent 触发方式。'],
    notes: ['需要同时Enable Agent 模式和对应 bot Channel。'],
  },
  'settings.agent.AGENT_ARCH': {
    ...
    valueNotes: [
      'single 适合简单场景，推理链路短。',
      'multi 适合复杂Analysis，支持技术Analysis、情报收集、风险评估等专项子 Agent 协作。',
    ],
    impact: ['影响 Agent Analysis流程、子 Agent 调度和最终Report结构。'],
  ...
  'settings.agent.AGENT_ORCHESTRATOR_MODE': {
    ...
    usage: 'quick：技术→决策；standard：技术→情报→决策；full：技术→情报→风险→决策；specialist：full + 每Strategy专项 Agent。',
    valueNotes: [
      '模式越完整，Analysis越深入，但Latency和 token 消耗也越大。',
      'specialist 模式会为每个Strategy启动独立 Agent。',
    ],
    impact: ['影响多 Agent Analysis流程的深度和资源消耗。'],
    notes: ['该字段仅在 AGENT_ARCH=multi 时生效。'],
  },
  'settings.agent.AGENT_ORCHESTRATOR_TIMEOUT_S': {
    title: 'Agent 超时',
    summary: 'Agent 执行的Total享超时预算（秒）。',
    usage: 'single 模式下作为整体 ReAct 循环超时；multi 模式下作为协作 pipeline 总超时。设为 0 禁用超时。',
    valueNotes: [
      '超时后 Agent 会返回Completed的部分Result。',
      'Analysis多只Stock或 specialist 模式建议适当调高。',
    ],
    impact: ['影响 Agent Analysis的最大等待Time。'],
    notes: ['超时不影响其他Stock的Analysis流程。'],
  },
  'settings.agent.AGENT_RISK_OVERRIDE': {
    title: '风险 Agent No决权',
    summary: '允许风险 Agent 在检测到关键风险信号时No决买入信号。',
    usage: '开启后，full/specialist 模式中的风险 Agent 可将买入建议降级为观望或卖出。',
    valueNotes: ['仅在 AGENT_ORCHESTRATOR_MODE 包含风险Phase时生效。'],
    impact: ['影响最终投资建议的风险保守程度。'],
    notes: ['Close后风险 Agent 的意见仅作参考，不会No决决策。'],
  },
  'settings.agent.DEEP_RESEARCH': {
    ...
    valueNotes: [
      '预算覆盖规划、后续研究和最终综合所有Phase。',
      '预算越高，研究越深入，但消耗也越大。',
    ],
    impact: ['影响 Deep Research 功能的深度和Latency。'],
    notes: ['Deep Research 在 Agent 模式下可用。'],
  },
  'settings.agent.AGENT_MEMORY_ENABLED': {
    title: 'Agent 记忆系统',
    summary: 'Enable记忆与校准系统，跟踪 Agent 预测准确率并调整置信度。',
    usage: '开启后，系统会记录每次预测Result，与后续实际走势对比，用于校准未来Analysis的置信度。',
    valueNotes: ['记忆数据需要积累一定量后才产生校准效果。'],
    impact: ['影响 Agent 置信度校准和长期Analysis质量。'],
    notes: ['需要配合Backtest功能使用效果更佳。'],
  },
  'settings.agent.AGENT_SKILL_AUTOWEIGHT': {
    title: 'Strategy自动权重',
    summary: '根据HistoryBacktest表现自动调整Strategy权重。',
    usage: '开启后，系统按各Strategy的HistoryBacktest准确率加权综合信号。',
    valueNotes: ['依赖Backtest数据；Backtest记录不足时可能无法有效加权。'],
    impact: ['影响多Strategy综合时的信号权重分配。'],
    notes: ['需要先开启Backtest功能并积累足够的Backtest数据。'],
  },
  'settings.agent.AGENT_SKILL_ROUTING': {
    title: 'Strategy路由模式',
    summary: '控制Strategy选择方式。',
    usage: 'auto 模式根据市场环境自动检测并选择相关Strategy；manual 模式仅使用 AGENT_SKILLS 中手动指定的Strategy。',
    valueNotes: ['auto 模式会根据市场Status（牛市/熊市/震荡等）动态调整Strategy选择。'],
    impact: ['影响 Agent Analysis时的Strategy覆盖面。'],
    notes: ['manual 模式下需要确保 AGENT_SKILLS 已正确Configuration。'],
  },
  'settings.agent.context_compression': {
    title: 'Ask AIContext compression',
    summary: '控制Ask AI可见对话History的滚动摘要压缩，默认Close以保持既有行为。',
    usage: 'AGENT_CONTEXT_COMPRESSION_ENABLED 开启后，仅压缩同一 session_id 下用户可见的 user/assistant 文本History；profile 控制默认触发Threshold和保留轮次。',
    valueNotes: [
    ...
    impact: ['降低长Ask AI会话的 token 消耗，但可能让更早的对话细节以摘要形式参与后续回答。'],
    notes: [
      '该功能不处理 provider trace、thinking blocks、tool calls 或 tool results，也不改变同轮工具调用透传。',
      '该Configuration只影响Ask AI可见History压缩，不改变 LLM provider、Models、Base URL、Save清理或运行时优先级语义。',
    ],
  },
  'settings.agent.event_monitor': {
    title: '事件监控',
    summary: '在定时模式下Enable后台事件监控，定期轮询AlertsRule。',
    usage: 'AGENT_EVENT_MONITOR_ENABLED 开启后台监控；AGENT_EVENT_MONITOR_INTERVAL_MINUTES 设定轮询间隔（分钟）。',
    valueNotes: [
      '仅在定时（schedule）模式下生效。',
      '轮询间隔过短可能增加 API 调用频率。',
    ],
    impact: ['影响后台Alerts检测频率和Notification推送时机。'],
    notes: ['需要配合AlertsRule使用。'],
  },
  'settings.agent.EVENT_ALERT_RULES_JSON': {
    title: '事件AlertsRule（Legacy JSON）',
    summary: '通过 JSON 数组Configuration基础Price和VolumeAlertsRule。',
    usage: 'JSON 数组格式，每条Rule包含 alert_type、stock_code 和条件字段。仅支持 price_cross、price_change_percent 和 volume_spike 三种基础Type。',
    valueNotes: [
      '技术Indicators、自Screening、Portfolio、大盘红绿灯等高级Rule需通过 Alert API 或 Web Alert center管理，不在此 JSON 中Configuration。',
      'Rule在定时模式下由事件监控定期评估。',
    ],
    impact: ['影响后台Alerts检测和Notification推送。'],
    notes: ['该字段为 Legacy Configuration方式，高级Rule请使用Alert center。'],
  },
  // ------------------------------------------------------------------
  ...
  'settings.backtest.BACKTEST_ENABLED': {
    title: 'Backtest开关',
    summary: 'Enable或CloseHistoryAnalysisBacktest功能。',
    usage: '开启后，系统会定期将HistoryAnalysisResult与后续实际走势对比，评估Strategy准确率。',
    valueNotes: [
      'Backtest数据用于Strategy自动权重（AGENT_SKILL_AUTOWEIGHT）和记忆校准。',
      'CloseBacktest不影响已有Backtest记录，但会停止新Backtest评估。',
    ],
    impact: ['影响Strategy权重校准、记忆校准和BacktestReport生成。'],
    notes: ['Agent Strategy自动权重功能依赖Backtest数据。'],
  },
  'settings.backtest.eval_params': {
    title: 'Backtest评估参数',
    summary: '控制BacktestEvaluation window、最小记录年龄和Neutral回报带的参数组。',
    usage: 'BACKTEST_EVAL_WINDOW_DAYS 设定Evaluation window（交易日数）；BACKTEST_MIN_AGE_DAYS 仅评估创建Time超过此天数的记录；BACKTEST_NEUTRAL_BAND_PCT 设定Neutral回报带百分比。',
    valueNotes: [
      'Evaluation window越长，包含的交易日越多，Result越稳定但延迟越高。',
      'BACKTEST_MIN_AGE_DAYS 用于排除过新的Analysis记录，等待足够后续走势数据。',
      'BACKTEST_NEUTRAL_BAND_PCT 内的涨跌标记为Neutral，不判定为正确或Error。',
    ],
    impact: ['影响BacktestResult的判定标准、覆盖范围和Strategy权重计算。'],
    notes: ['三个参数Total同影响BacktestResult，建议组合调整。'],
  },
  'settings.backtest.BACKTEST_ENGINE_VERSION': {
    title: 'Backtest引擎版本',
    summary: 'Backtest引擎版本标签。',
    usage: '一般无需修改；版本标签用于标识当前使用的Backtest逻辑版本。',
    valueNotes: ['不同版本可能使用不同的评估算法或判定Rule。'],
    impact: ['影响Backtest评估算法和Result。'],
    notes: ['除非明确要求切换版本，No则保持默认。'],
  },
  // ------------------------------------------------------------------
  ...
  'settings.report.REPORT_SUMMARY_ONLY': {
    title: '仅推送摘要',
    summary: '只推送Analysis摘要，不推送个股Details。适合跟踪大量Stock时快速概览。',
    usage: '开启后，Notification只包含整体摘要信息；Close后包含每只Stock的详细Analysis。',
    valueNotes: ['跟踪Stock较多时开启可减少Notification篇幅。'],
    impact: ['影响Notification推送的内容详细程度。'],
    notes: ['不影响 Web 端Report查看。'],
  },
  'settings.report.REPORT_SHOW_LLM_MODEL': {
    title: 'Report显示Models名',
    summary: '在Reportpage脚展示本次Analysis使用的 LLM ModelsName。',
    usage: '开启后，NotificationReportpage脚会显示Models标识；Close后隐藏。',
    valueNotes: ['仅影响Report渲染，不改变Models选择、运行时路由或迁移行为。'],
    impact: ['影响NotificationReport中的Models信息可见性。'],
    notes: ['用于排查Models切换问题时可以临时开启。'],
  },
  'settings.report.REPORT_TEMPLATES_DIR': {
    title: 'Report模板目录',
    summary: 'Jinja2 Report模板的存放目录。',
    usage: '填写相对于项目根目录的路径；目录内放置 Jinja2 模板文件。',
    valueNotes: ['需要同时开启 REPORT_RENDERER_ENABLED 才会使用模板渲染。'],
    impact: ['影响Report渲染使用的模板Source。'],
    notes: ['自定义模板需要符合 Jinja2 语法并包含必要的变量占位。'],
  },
  'settings.report.REPORT_RENDERER_ENABLED': {
    title: 'Report渲染引擎',
    summary: 'Enable Jinja2 模板渲染引擎处理Report输出。',
    usage: '默认Close；开启后Report会通过 Jinja2 模板渲染，支持自定义格式。',
    valueNotes: ['默认Close以确保零回归风险。'],
    impact: ['影响Report最终输出格式。'],
    notes: ['开启前请确认 REPORT_TEMPLATES_DIR 中有有效的模板文件。'],
  },
  'settings.report.REPORT_INTEGRITY_ENABLED': {
    title: 'Report完整性校验',
    summary: 'LLM 输出后校验必填字段，缺失时Retry或使用占位符。',
    usage: '开启后系统会检查ReportYesNo包含必要的Analysis字段；REPORT_INTEGRITY_RETRY 控制Retry次数。',
    valueNotes: [
      '校验failed的字段会用占位符填充。',
      'Retry会增加 LLM 调用次数和Latency。',
    ],
    impact: ['影响Report完整度和 LLM 调用次数。'],
    notes: ['REPORT_INTEGRITY_RETRY=0 时不Retry，仅用占位符。'],
  },
  'settings.report.REPORT_HISTORY_COMPARE_N': {
    title: 'History信号对比',
    summary: '展示每只Stock最近 N 次Analysis的信号对比。设为 0 Close。',
    usage: '开启后，Report中会展示最近 N 次Analysis信号的对比表格。',
    valueNotes: ['N 越大对比范围越广，但表格越长。'],
    impact: ['影响Report中History信号对比部分的展示。'],
    notes: ['设为 0 Close该功能。'],
  },
  'settings.report.SINGLE_STOCK_NOTIFY': {
    title: '逐股即时推送',
    summary: '每completed一只StockAnalysis后立即推送，而不Yes等Allcompleted后批量推送。',
    usage: '开启后，每只StockAnalysiscompleted后独立SendNotification；Close后汇总Send。',
    valueNotes: ['开启后Notification更及时，但推送频率更高。'],
    impact: ['影响Notification推送时机和频率。'],
    notes: ['跟踪大量Stock时可能产生较多Notification消息。'],
  },
  'settings.report.MERGE_EMAIL_NOTIFICATION': {
    title: '合并邮件Notification',
    summary: '将个股Analysis与Market review合并为一封邮件Send。',
    usage: '开启后，个股Analysis和Market review会合并在同一封邮件中Send。',
    valueNotes: ['仅在同时Enable了个股Analysis和Market review时有效。'],
    impact: ['影响邮件Notification的封数和内容组织。'],
    notes: ['Close后个股Analysis和Market review会分别Send邮件。'],
  },
  // ------------------------------------------------------------------
  // Notification routing
  // ------------------------------------------------------------------
  'settings.notification.channel_routing': {
    title: 'NotificationChannel路由',
    summary: '为不同Type的Notification指定Target推送Channel。',
    usage: '三个路由字段分别控制Report推送、Alerts推送和系统Error推送的TargetChannel。使用英文逗号分隔Channel名；留空则推送到所有已ConfigurationChannel。',
    valueNotes: [
      'NOTIFICATION_REPORT_CHANNELS 控制日常AnalysisReport推送Channel。',
      'NOTIFICATION_ALERT_CHANNELS 控制事件Alerts推送Channel。',
      'NOTIFICATION_SYSTEM_ERROR_CHANNELS 控制系统Error推送Channel。',
      '可用Channel取决于已Configuration的NotificationChannel（如 email、feishu、telegram 等）。',
    ],
    impact: ['影响不同NotificationType的推送Target。'],
    notes: ['指定的Channel必须Completed对应Configuration，No则不会生效。'],
  },
  'settings.notification.dedup': {
    title: 'Notification去重与Cooldown',
    summary: '控制静态Notification的去重Time窗口和CooldownTime。',
    usage: 'NOTIFICATION_DEDUP_TTL_SECONDS 设定去重Time窗口，同一去重 key 在窗口内只推送一次；NOTIFICATION_COOLDOWN_SECONDS 设定CooldownTime，同一Cooldown key 在窗口内只推送一次。',
    valueNotes: [
      '两个机制独立生效，互不影响。',
      '都设为 0 时Close对应去重或Cooldown功能。',
    ],
    impact: ['影响Notification推送频率和重复控制。'],
    notes: ['动态Notification（如Alerts）有独立的触发和Cooldown逻辑。'],
  },
  'settings.notification.quiet_hours': {
    title: '静默时段',
    summary: '在指定Time段内抑制Notification推送。',
    usage: 'NOTIFICATION_QUIET_HOURS 使用 HH:MM-HH:MM 格式，支持跨夜；NOTIFICATION_TIMEZONE 指定对应时区。',
    valueNotes: [
      '留空Close静默时段。',
      '时区留空时使用系统本地时区。',
    ],
    impact: ['影响Notification推送的时段。'],
    notes: ['静默时段内的静态Notification会被抑制并跳过，不会在时段结束后补发。'],
  },
  'settings.notification.MIN_SEVERITY': {
    title: '最低Notification等级',
    summary: '过滤低于指定等级的静态Notification。',
    usage: '设为 warning 时，只有 warning 及以上等级的Notification会被推送；留空保留当前行为。',
    valueNotes: ['等级从低到高依次为 info、warning、error、critical。'],
    impact: ['影响静态Notification的推送量。'],
    notes: ['动态Notification（如Alerts）有独立的事件和等级评估。'],
  },
  'settings.notification.DAILY_DIGEST_ENABLED': {
    title: '每日摘要（预留）',
    summary: '预留功能开关，当前不会Send每日摘要。',
    usage: '该字段为 P4 预留功能，当前开启后不会产生任何效果。',
    valueNotes: ['保留用于后续每日摘要聚合推送功能。'],
    impact: ['当前无实际影响。'],
    notes: ['未来版本Enable后会聚合当日Notification为一条摘要推送。'],
  },
  // ------------------------------------------------------------------
  // System runtime
  // ------------------------------------------------------------------
  ...
  'settings.system.DEBUG': {
    ...
    usage: '开启后会输出更多内部Status和调试信息。',
    valueNotes: ['生产环境建议Close以减少日志量。'],
    impact: ['影响日志详细程度和运行时行为。'],
    notes: ['调试模式可能影响性能，不建议长期开启。'],
  },
  'settings.system.MAX_WORKERS': {
    title: '最大并发线程数',
    summary: '控制同时执行的StockAnalysis线程Count。',
    usage: 'Settings并发Analysis的工作线程数；数值越高并发越高，但 API 限流风险也越大。',
    valueNotes: [
      '建议保持较低值以避免触发数据源或 LLM API 的频率限制。',
      '受限于 API Key 配额和网络带宽。',
    ],
    impact: ['影响Analysis总Latency和 API 调用频率。'],
    notes: ['并发过高可能导致 API 返回限流Error。'],
  },
  'settings.system.ANALYSIS_DELAY': {
    title: 'Analysis间隔',
    summary: '控制每只StockAnalysis之间的间隔秒数，用于限速。',
    usage: '设为 0 无间隔；设为正值时每completed一只Stock后等待指定秒数再Analysis下一只。',
    valueNotes: ['适合在 API 有严格频率限制时使用。'],
    impact: ['影响Analysis总Latency。'],
    notes: ['总Latency ≈ Stock数 × 单股Latency + (Stock数-1) × ANALYSIS_DELAY。'],
  },
  'settings.system.SAVE_CONTEXT_SNAPSHOT': {
    title: 'SaveAnalysis上下文快照',
    summary: '控制YesNo将AnalysisHistory的整份 context_snapshot 持久化到数据库。',
    usage: '默认开启。Close后，新History不会Save enhanced_context、market_phase_summary、AnalysisContextPack overview 或运行Diagnostics快照等 context_snapshot 内容。',
    valueNotes: [
      'Close后，新History的HistoryDetails、completed 任务Status和 Web Reportpage无法读取低敏输入数据块摘要。',
      '该开关不Close当次 AnalysisContextPack 构建，也不Close LLM Prompt 中的低敏 pack summary。',
      'CLI 的 --no-context-snapshot 与设为 false 的持久化效果一致。',
    ],
    impact: ['影响History透明度、Backtest/Diagnostics可用的上下文快照信息和 Web Reportpage的数据Source摘要。'],
    notes: ['若需要完全Close P3-P5 pack 接入，需要回滚相关Code；当前没有运行时 pack 总开关。'],
  },
  'settings.system.market_review': {
    title: '大盘Analysis',
    summary: '控制大盘Analysis功能的开关、覆盖市场和配色方案。',
    usage: 'MARKET_REVIEW_ENABLED 开启大盘Analysis；MARKET_REVIEW_REGION 选择市场（cn/hk/us/both）；MARKET_REVIEW_COLOR_SCHEME 选择配色。',
    valueNotes: [
      'cn 覆盖 A 股，hk 覆盖港股，us 覆盖美股，both 覆盖All。',
      '配色方案影响大盘Report中指数涨跌的颜色显示：green_up 为绿涨红跌，red_up 为红涨绿跌。',
    ],
    impact: ['影响AnalysisReport中大盘概览部分的内容和视觉呈现。'],
    notes: ['大盘Analysis依赖对应市场的指数数据源可用性。'],
  },
};
...
export function getSettingsHelpContent(
  ...
  const localized = getPreferredHelpMap(locale)[helpKey] ?? settingsHelpEnUS[helpKey] ?? settingsHelpZhCN[helpKey];
  if (localized) {
    return localized;
  }
