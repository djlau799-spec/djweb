...
export const useStockPoolStore = create<StockPoolState>((set, get) => ({
  ...
  submitAnalysis: async (options) => {
    ...
    if (selectionSource !== 'autocomplete' && isObviouslyInvalidStockQuery(stockCodeInput)) {
      set({ inputError: 'Enter a valid stock code or stock name', duplicateError: null });
      return;
    }
    ...
    try {
      ...
      if (error instanceof DuplicateTaskError) {
        set({
          duplicateError: `Stock ${error.stockCode} is being analyzed，please wait until it completes`,
        });
        return;
      }
  ...
  syncTaskFailed: (task) => {
    ...
    set({ error: getParsedApiError(task.error || 'Analysisfailed') });
  },
